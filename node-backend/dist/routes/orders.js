...
async function adjustWallet(userId, delta, reference, txnType) {
...
router.get('/', authenticate, async (req, res) => {
    ...
    if (user.userType === 'owner') {
        ...
        const tiffinIds = ownerTiffins.map((t) => t.id);
    ...
    if (user.userType === 'delivery') {
        ...
        const result = await prisma.order.findMany({
            where: {
                ...
                status: { in: ['ready_for_delivery', 'picked_up'] },
...
router.post('/', authenticate, requireRole('customer'), async (req, res) => {
    ...
    if (!tiffin)
        return res.status(400).json({ detail: 'Invalid tiffin' });
...
router.post('/:id/update_status/', authenticate, async (req, res) => {
    ...
    if (user.userType === 'owner') {
        ...
        if (!owner || !tiffin || tiffin.ownerId !== owner.id)
            return res.status(403).json({ detail: 'Not owner' });
    }
    ...
    if (previous === 'pending' && ['confirmed', 'preparing', 'ready_for_delivery', 'picked_up', 'delivered'].includes(newStatus)) {
        ...
        if (tiffin) {
            ...
            if (owner)
                await adjustWallet(owner.userId, order.totalPrice, `ORDER:${order.id}`, 'credit for tiffin');
        }
    }
