...
router.post('/', authenticate, requireRole('owner'), upload.single('image'), async (req, res) => {
    ...
    if (!owner)
        return res.status(400).json({ detail: 'Tiffin owner profile not found' });
...
router.patch('/:id/', authenticate, requireRole('owner'), upload.single('image'), async (req, res) => {
    const tiffin = await prisma.tiffin.findUnique({ where: { id: req.params.id } });
    if (!tiffin)
        return res.status(404).json({ detail: 'Not found' });
    const owner = await prisma.tiffinOwner.findUnique({ where: { userId: req.user.id } });
    if (!owner || tiffin.ownerId !== owner.id)
    ...
    if (is_available !== undefined)
        data.isAvailable = String(is_available) === 'true';
    if (req.file)
        data.image = path.join('uploads', path.basename(req.file.path));
...
//# sourceMappingURL=tiffins.js.map