...
router.get('/', authenticate, async (req: AuthRequest, res) => {
  ...
  if (user.userType === 'owner') {
    ...
    const tiffinIds = (await prisma.tiffin.findMany({ where: { ownerId: owner.id }, select: { id: true } })).map((t: any) => t.id);
...
router.post('/:id/accept/', authenticate, requireRole('delivery'), async (req: AuthRequest, res) => {
  const delivery = await prisma.delivery.findUnique({ where: { id: req.params.id } });
  if (!delivery) return res.status(404).json({ detail: 'Not found' });
  if (delivery.deliveryBoyId) return res.status(400).json({ error: 'Delivery already assigned.' });

  const dBoy = await prisma.deliveryBoy.findUnique({ where: { userId: req.user!.id } });
  if (!dBoy) return res.status(400).json({ detail: 'Delivery boy profile missing' });
...
router.post('/:id/update_status/', authenticate, async (req: AuthRequest, res) => {
  const newStatus = req.body.status as DeliveryStatus;
  const allowed: DeliveryStatus[] = ['pending', 'accepted', 'picked_up', 'delivered', 'cancelled'];
